...
public class C_QSortOptimized {
    ...
    <T extends Comparable<T>> AbstractMap.SimpleEntry<Integer, Integer> repeats(T[] arr, int left, int right) {
        var pivot = arr[(left + right) / 2];
        var equal = left;
        while (equal <= right)
            if (arr[equal].compareTo(pivot) < 0)
                swap(arr, equal++, left++);
            else if (arr[equal].compareTo(pivot) > 0)
